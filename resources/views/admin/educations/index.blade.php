...
@section('page-title', 'تحصیلات و دوره‌ها')

@section('content')
    <div class="a-card">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
            <h3 class="flex items-center gap-2 font-black text-slate-800">
                <i class="fa-solid fa-graduation-cap text-brand-600"></i> لیست تحصیلات و دوره‌ها
                <span class="a-badge bg-slate-100 text-slate-500">{{ $items->count() }}</span>
            </h3>
            <a href="{{ route('admin.educations.create') }}" class="a-btn-primary !px-4 !py-2 text-xs">
                <i class="fa-solid fa-plus"></i> مورد جدید
            </a>
        </div>
            ...
            <div class="flex flex-wrap items-center gap-4 border-b border-slate-100 px-6 py-4 transition last:border-0 hover:bg-slate-50">
                ...
                <div class="min-w-0 flex-1">
                    <div class="mb-1 flex flex-wrap items-center gap-2">
                        <p class="font-bold text-slate-800">{{ $item->degree }}</p>
                        @unless ($item->is_active)
                            <span class="a-badge-off">غیرفعال</span>
                        @endunless
                    </div>
                    <p class="truncate text-xs text-slate-400">{{ $item->institution }} @if($item->period) · {{ $item->period }} @endif</p>
                </div>

                <span class="a-badge bg-slate-100 text-slate-500">ترتیب: {{ $item->sort_order }}</span>

                <div class="flex items-center gap-2">
                    <a href="{{ route('admin.educations.edit', $item) }}"
                    ...
                    <form method="POST" action="{{ route('admin.educations.destroy', $item) }}"
                          onsubmit="return confirm('آیا از حذف «{{ $item->degree }}» مطمئن هستید؟')">
                        @csrf
                        @method('DELETE')
        ...
        @empty
            <div class="py-14 text-center">
                ...
                <p class="mb-4 text-sm text-slate-400">هنوز موردی ثبت نشده است.</p>
                <a href="{{ route('admin.educations.create') }}" class="a-btn-primary !px-4 !py-2 text-xs">افزودن اولین مورد</a>
            </div>
        @endforelse
    </div>
@endsection

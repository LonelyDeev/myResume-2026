...
@section('page-title', 'لینک‌های شبکه‌های اجتماعی')
    ...
    <div class="a-card">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
            <div>
                <h3 class="flex items-center gap-2 font-black text-slate-800">
                    <i class="fa-solid fa-share-nodes text-brand-600"></i> لیست لینک‌ها
                    <span class="a-badge bg-slate-100 text-slate-500">{{ $items->count() }}</span>
                </h3>
                <p class="mt-1 text-xs text-slate-400">این لینک‌ها در هدر و سکشن تماس صفحه اصلی نمایش داده می‌شوند.</p>
            </div>
            <a href="{{ route('admin.social-links.create') }}" class="a-btn-primary !px-4 !py-2 text-xs">
                <i class="fa-solid fa-plus"></i> لینک جدید
            </a>
        </div>
            ...
            <div class="flex flex-wrap items-center gap-4 border-b border-slate-100 px-6 py-4 transition last:border-0 hover:bg-slate-50">
                ...
                <div class="min-w-0 flex-1">
                    <div class="mb-1 flex flex-wrap items-center gap-2">
                        <p class="font-bold text-slate-800">{{ $item->platform }}</p>
                        @unless ($item->is_active)
                            <span class="a-badge-off">غیرفعال</span>
                        @endunless
                    </div>
                    <p dir="ltr" class="truncate text-left text-xs text-slate-400">{{ $item->url }}</p>
                </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <form method="POST" action="{{ route('admin.social-links.destroy', $item) }}"
                          onsubmit="return confirm('آیا از حذف «{{ $item->platform }}» مطمئن هستید؟')">
                        @csrf
                        @method('DELETE')
        ...
        @empty
            <div class="py-14 text-center">
                <i class="fa-regular fa-share-from-square mb-3 block text-4xl text-slate-200"></i>
                <p class="mb-4 text-sm text-slate-400">هنوز لینکی ثبت نشده است.</p>
                <a href="{{ route('admin.social-links.create') }}" class="a-btn-primary !px-4 !py-2 text-xs">افزودن اولین لینک</a>
            </div>
        @endforelse
    </div>
@endsection
